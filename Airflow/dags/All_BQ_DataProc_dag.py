...
Year_Month = '{{ (execution_date + macros.dateutil.relativedelta.relativedelta(months=-2)).strftime("%Y-%m") }}' 
Year_folder = '{{ (execution_date + macros.dateutil.relativedelta.relativedelta(months=-2)).strftime("%Y") }}'

PYSPARK_FILE = "code/Spark-run-ALL.py"
PYSPARK_JOB = {
    ...
    {   "main_python_file_uri": f"gs://{BUCKET_NAME}/{PYSPARK_FILE}",
        "args": [f"--year_month={Year_Month}",f"--year={Year_folder}"],
